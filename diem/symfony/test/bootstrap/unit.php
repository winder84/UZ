...
sfConfig::set('sf_test_cache_dir', _sys_get_temp_dir().'/sf_test_project');

// remove all test cache
sf_unit_test_shutdown();

// create test cache dir
$sf_root_dir = _sys_get_temp_dir().'/sf_test_project';
...
function sf_unit_test_shutdown()
{
  ...
  $sessions = glob(_sys_get_temp_dir().'/sessions*');
  $tmp_files = glob(_sys_get_temp_dir().'/sf*');
